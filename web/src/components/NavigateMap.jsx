...
const createImportantPointIcon = (name, isSelected = false, isCustomPoint = false) => {
    const bgColor = isCustomPoint ? '#9333ea' : (isSelected ? '#f59e0b' : '#ff4444');
    const starColor = isCustomPoint ? '📍' : '⭐';
...
// Path data - define connections between nodes
const PATH_DATA = [
    ...
    { id: 54, name: 'street41', type: 'street', nodes: [72, 130, 131] },
];
...
const NavigateMap = () => {
    ...
    // Accept either placeName or coordinates from previous page
    const { placeName, coordinates, customName } = location.state || {};
    ...
    useEffect(() => {
        if (coordinates && Array.isArray(coordinates) && coordinates.length > 0) {
            ...
            // Create custom points from coordinates
            const newCustomPoints = coordinates.map((coord, index) => ({
                id: `custom-${Date.now()}-${index}`,
                lat: coord[0],
                lng: coord[1],
                name: customName ? `${customName} ${index + 1}` : `Location ${index + 1}`,
                isCustom: true
            }));

            setCustomPoints(newCustomPoints);

            // Set the first point as target
            if (newCustomPoints.length > 0) {
                const firstPoint = newCustomPoints[0];
                setTargetPoint(firstPoint);
                setSelectedImportantPoint(firstPoint.id);

                // Find nearest node to this point
                const nearest = findNearestNode(firstPoint.lat, firstPoint.lng);
                setDestinationNode(nearest);

                // Center map on the point
                if (map) {
                    map.setView([firstPoint.lat, firstPoint.lng], 18);
                }
            }
        }
    }, [coordinates, customName, map]);

    // Handle placeName from previous page (for backward compatibility)
    useEffect(() => {
        if (!placeName || !map || customPoints.length > 0) return;

        console.log("Navigating to place:", placeName);

        const matchedPoint = IMPORTANT_POINTS.find(point =>
            point.name.toLowerCase() === placeName.toLowerCase() ||
            point.name.toLowerCase().includes(placeName.toLowerCase())
        );

        if (matchedPoint) {
            console.log("Found matching point:", matchedPoint);
            setTargetPoint(matchedPoint);
            setSelectedImportantPoint(matchedPoint.id);

            const nearest = findNearestNode(matchedPoint.lat, matchedPoint.lng);
            setDestinationNode(nearest);

            map.setView([matchedPoint.lat, matchedPoint.lng], 18);

            setTimeout(() => {
                map.eachLayer((layer) => {
                    if (layer instanceof L.Marker && layer.getPopup()?.getContent()?.includes(matchedPoint.name)) {
                        layer.openPopup();
                    }
                });
            }, 500);
        }
    }, [placeName, map, customPoints]);

    // Calculate path when both nodes are set
    useEffect(() => {
        if (nearestNode && destinationNode && nearestNode !== destinationNode) {
    ...
    // Combine all points to display
    const allPointsToDisplay = [
        ...IMPORTANT_POINTS,
        ...customPoints
    ];

    const displayName = customPoints.length > 0
        ? (customName || `${customPoints.length} Locations`)
        : (placeName || "Selected Location");

    return (
        <div className="map-node-mapper-container">
            <div className="map-node-mapper-header">
                <h1>🗺️ Navigation to {displayName}</h1>
                {targetPoint && (
                    <div className="navigation-info" style={{
                        background: '#e8f5e9',
                        padding: '8px 16px',
                        borderRadius: '8px',
                        marginTop: '8px'
                    }}>
                        <strong>📍 Destination:</strong> {targetPoint.name}
                        {destinationNode && ` (Nearest Node: #${destinationNode})`}
                        {targetPoint.lat && (
                            <div style={{ fontSize: '12px', marginTop: '4px' }}>
                                Coordinates: {targetPoint.lat.toFixed(6)}, {targetPoint.lng.toFixed(6)}
                            </div>
                        )}
                    </div>
                )}
                {customPoints.length > 0 && (
                    <div style={{
                        ...
                        <strong>📍 {customPoints.length} Custom Location(s):</strong>
            ...
            <div className="map-node-mapper-main">
                ...
                <div className="map-container">
                    ...
                    >
                            ...
                            >
                                <Popup>
                                    <div className="map-node-popup">
                                        <h3>Your Location (Start Point)</h3>
                        ...
                        {/* Important Points Layer (Predefined) */}
                        {IMPORTANT_POINTS
                            .filter(point => {
                                // Only filter if placeName exists, otherwise show all points
                                if (!placeName) return true;
                                return point.name.toLowerCase().includes(placeName.toLowerCase());
                            })
                            .map(point => (
                                <Marker
                        ...
                        {/* Custom Points Layer (from coordinates) */}
                        {customPoints.map(point => (
                            <Marker
                                key={point.id}
                                position={[point.lat, point.lng]}
                                icon={createImportantPointIcon(point.name, selectedImportantPoint === point.id, true)}
                                eventHandlers={{
                                    click: () => {
                                ...
                                <Popup>
                                    <div className="map-important-point-popup" style={{ minWidth: '200px' }}>
                                        ...
                                        <div style={{
                                            ...
                                            <div><strong>Type:</strong> Custom Location</div>
                                            <div><strong>Latitude:</strong> {point.lat.toFixed(6)}</div>
                                            <div><strong>Longitude:</strong> {point.lng.toFixed(6)}</div>
                                        </div>
                                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
