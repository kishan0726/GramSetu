...
const MapScreen = () => {
  ...
  // HTML content for WebView with all custom marker functions
  const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        ...
        .map-path-badge.highway { background: #fee2e2; color: #dc2626; }
        .map-path-badge.mainRoad { background: #dbeafe; color: #3b82f6; }
        .map-path-badge.street { background: #d1fae5; color: #10b981; }
        .map-path-badge.footpath { background: #f3f4f6; color: #6b7280; }
        .map-path-badge.track { background: #ffedd5; color: #b45309; }
        
        .map-path-type-badge-popup {
          display: inline-block;
          padding: 2px 8px;
          background: #f3f4f6;
          border-radius: 12px;
    ...
    <body>
      ...
      <script>
        ...
        // Create map
        const map = L.map('map').setView([21.771, 69.452], 16);
        ...
        IMPORTANT_POINTS.forEach(point => {
          const marker = L.marker([point.lat, point.lng], {
            icon: createImportantPointIcon(point.name, false)
          }).addTo(map);
          
          marker.bindPopup(\`
            <div class="map-important-point-popup" style="min-width: 200px;">
              <h3 style="color: #ff4444; margin: 0 0 8px 0; display: flex; align-items: center; gap: 4px;">
              ...
              <div style="display: flex; gap: 8px; justify-content: center;">
                <button class="map-popup-btn" onclick="window.ReactNativeWebView.postMessage(JSON.stringify({type: 'setDestinationFromPoint', pointId: \${point.id}, lat: \${point.lat}, lng: \${point.lng}}))">
                  Set as Destination
                </button>
                <button class="map-popup-btn" onclick="window.ReactNativeWebView.postMessage(JSON.stringify({type: 'zoomToPoint', lat: \${point.lat}, lng: \${point.lng}}))">
                  Zoom Here
                </button>
              </div>
            </div>
        ...
        PATH_DATA.forEach(path => {
          ...
          if (coordinates.length > 1) {
            ...
            polyline.bindPopup(\`
              <div class="map-path-popup">
                <strong style="color: \${style.color}">\${path.name}</strong>
                <div class="map-path-type-badge-popup">
                  \${path.type} • \${style.weight}px
                </div>
                <div class="map-path-stats">
                  <span>📏 \${length < 1000 ? Math.round(length) + 'm' : (length/1000).toFixed(2) + 'km'}</span>
        ...
        // Add Nodes
        NODE_DATA.forEach(node => {
          const isPathNode = PATH_DATA.some(path => path.nodes.includes(node.id));
          
          const marker = L.marker([node.lat, node.lng], {
            icon: createNodeIcon(node.id, false, isPathNode, false, false)
          }).addTo(map);
        ...
        // Function to set satellite view
        ...
        window.setStartMarker = (lat, lng) => {
          ...
          startMarker = L.marker([lat, lng], { icon: createSpecialIcon('start') }).addTo(map);
        ...
        window.setDestinationMarker = (lat, lng) => {
          ...
          endMarker = L.marker([lat, lng], { icon: createSpecialIcon('end') }).addTo(map);
          endMarker.bindPopup('Destination');
        };

        // Function to draw shortest path
        window.drawShortestPath = (pathNodes) => {
          if (shortestPathLayer) map.removeLayer(shortestPathLayer);
          ...
          if (coordinates.length > 1) {
            shortestPathLayer = L.polyline(coordinates, pathStyles.shortestPath).addTo(map);
            shortestPathLayer.bindPopup('Shortest Path');
  ...
  const handleMessage = (event) => {
    try {
      ...
      switch (data.type) {
        ...
        case 'setDestination':
          setDestinationNode(data.nodeId);
          // Find node coordinates
          const node = NODE_DATA.find(n => n.id === data.nodeId);
          if (node) {
            webViewRef.current?.injectJavaScript(`
              window.setDestinationMarker(${node.lat}, ${node.lng});
            `);
          }
          break;

        case 'setDestinationFromPoint':
          setDestinationNode(findNearestNode(data.lat, data.lng));
          webViewRef.current?.injectJavaScript(`
            window.setDestinationMarker(${data.lat}, ${data.lng});
          `);
          break;
        ...
        case 'zoomToPoint':
          ...
          `);
          setSelectedImportantPoint(data.pointId);
          break;
      }
    } catch (error) {
  ...
  const calculateShortestPath = () => {
    ...
    if (nearestNode === destinationNode) {
      Alert.alert('Error', 'Start and destination are the same node');
      return;
    }
    ...
    if (result.path.length < 2) {
      Alert.alert('Error', 'No path found between these nodes');
      return;
    }
    ...
    webViewRef.current?.injectJavaScript(`
      window.drawShortestPath(${JSON.stringify(result.path)});
    `);
  };
  ...
  const handleSearchNode = () => {
    ...
    if (!isNaN(nodeId) && nodeId >= 1 && nodeId <= NODE_DATA.length) {
      ...
      Alert.alert('Error', 'Invalid node ID');
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading && (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading Village Map...</Text>
        </View>
      )}
      ...
      <View style={styles.routingPanel}>
        ...
        <View style={styles.buttonRow}>
          <TouchableOpacity
            ...
            onPress={calculateShortestPath}
            disabled={!nearestNode || !destinationNode}
          >
            <Icon name="directions" size={20} color="#fff" />
            <Text style={styles.buttonText}>Find Shortest Path</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.actionButton, styles.dangerButton]}
            onPress={clearRouting}
          >
        ...
        <View style={styles.pathDistance}>
          ...
          <Text style={styles.distanceValue}>{formatDistance(shortestPathDistance)}</Text>
        </View>
      </View>
      ...
      {showInfoPanel && (
        <ScrollView style={styles.infoPanel}>
          ...
          <View style={styles.statItem}>
            ...
            <Text style={styles.statValue}>{NODE_DATA.length}</Text>
          </View>

          <View style={styles.statItem}>
            <Text>Total Paths:</Text>
            <Text style={styles.statValue}>{PATH_DATA.length}</Text>
          </View>

          <View style={styles.statItem}>
            <Text>Connected Nodes:</Text>
            <Text style={styles.statValue}>{new Set(PATH_DATA.flatMap(p => p.nodes)).size}</Text>
          </View>

          <View style={styles.statItem}>
            <Text>Important Places:</Text>
            <Text style={[styles.statValue, { color: '#ff4444' }]}>{IMPORTANT_POINTS.length}</Text>
          </View>

          <View style={styles.statItem}>
            <Text>Total Path Length:</Text>
          ...
          {selectedNode && (
            <>
              <Text style={styles.infoSubtitle}>Selected Node #{selectedNode}</Text>
              {(() => {
                const node = NODE_DATA.find(n => n.id === selectedNode);
                return node ? (
                  <>
                    <Text>Lat: {node.lat.toFixed(6)}</Text>
                    <Text>Lng: {node.lng.toFixed(6)}</Text>
                    <Text style={styles.connectionsTitle}>Connected Paths:</Text>
                    {getNodeConnections(selectedNode).map((conn, idx) => (
                      <View key={idx} style={styles.connectionItem}>
                        <Text style={[styles.pathType, { color: getPathStyle(conn.pathType).color }]}>
                          {conn.pathName}
                        </Text>
                        <Text style={styles.connectionNodes}>
                          Node {conn.fromNode} ↔ {conn.toNode}
                        </Text>
                      </View>
                    ))}
                  </>
          ...
          {selectedPath && (
            <>
              <Text style={styles.infoSubtitle}>Selected Path</Text>
              {(() => {
                const path = PATH_DATA.find(p => p.id === selectedPath);
                const style = getPathStyle(path.type);
                return path ? (
                  <>
                    <Text style={[styles.pathName, { color: style.color }]}>{path.name}</Text>
                    <Text>Type: {path.type} • {style.weight}px</Text>
                    <Text>Length: {formatDistance(pathLengths[path.id] || 0)}</Text>
                    <Text>Nodes: {path.nodes.length}</Text>
                  </>
                ) : null;
              })()}
            </>
          ...
          <Text style={styles.infoSubtitle}>⭐ Important Places</Text>
          {IMPORTANT_POINTS.map(point => (
            <TouchableOpacity
              key={point.id}
              style={[
                styles.placeItem,
                selectedImportantPoint === point.id && styles.selectedPlace
              ]}
              onPress={() => {
                webViewRef.current?.injectJavaScript(`
            ...
            >
              <View style={styles.placeHeader}>
                <Text style={styles.placeName}>⭐ {point.name}</Text>
              </View>
